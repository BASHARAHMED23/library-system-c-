...
struct user {
	...
	void return_copy(int book_id) {
		bool removed = false;
		for (int i = 0; i < len; ++i) { // we want to remove the book which has been returned from the array of borrowed books 
...
struct library_system {
	...
	void user_borrow_book() { // we need user name and book name so we made a function for it 
		...
		if (!books[book_idx].borrow(user_id)) // we borrow the book by user id 
	...
	void print_who_borrowed_book_by_name() {
		...
		int book_idx = find_book_idx_by_name(book_name); // will return -1 or the index of the book 
